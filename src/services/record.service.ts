...
export class RecordService{
    ...
    async cadastrarNovasCategorias():Promise<any>{
        let response = await this.axios.get(`http://aquila-backend-records:3003/repositories/categorias`);
        return response.data;
    }

    async cadastrarNovosVideos():Promise<any>{
        let response = await this.axios.get(`http://aquila-backend-records:3003/repositories/videos`);
        return response.data;
    }
